<div class="acc-billing-item__top-line">
    @if(auth()->user()->isPartner())
        <h4 class="acc-billing-item__title">Umzug + Reinigung</h4>
    @else
...
<div class="acc-billing-item__characteristic">
    <p class="acc-billing-item__characteristic-txt">{{$additional_info->from->rooms}} Zimmer, {{$additional_info->from->floor}} Stock, {{$additional_info->cleaning}}, {{$additional_info->windows}} Fenster, {{$additional_info->shower_wc}}, {{$additional_info->bath_wc}}, {{$additional_info->wc}}</p>
    @if(auth()->user()->isPartner())
        @if($showactionbuttons)
            <div class="acc-billing-item__actions">
                <a class="acc-billing-item__btn-accept" href="{{route('partner.processProposal',[$proposal_id,'accepted'])}}">Annehmen<br> (Für {{ Setting::getByKey('system.price.'.auth()->user()->subscription_id.'.cost_transfer_cleaning') }} Chf)</a>
